<script>
  ...
  let expandedSections = $state({ frontOffice: true, backOffice: false, admin: false });
  ...
  const navSections = [
    ...
    },
    {
      ...
      title: 'Front Office',
      items: [
        { label: 'Antrian Mandiri (APM)', href: '/apm', icon: 'tablet' },
        ...
        { label: 'Rawat Inap', href: '/rawat-inap', icon: 'hospital-bed' },
  ...
  <div class="flex h-screen overflow-hidden bg-gray-50">
    ...
    >
      <div class="flex items-center h-16 px-4 border-b border-gray-200 {sidebarCollapsed ? 'justify-center' : 'gap-3'}">
        <div class="flex items-center justify-center w-9 h-9 rounded-lg bg-emerald-600 text-white flex-shrink-0">
        ...
        {#if !sidebarCollapsed}
          <div class="flex flex-col min-w-0">
            <span class="text-sm font-bold text-gray-900 truncate">SMARTHEALTH</span>
            <span class="text-[10px] text-gray-400 truncate">Sistem Informasi Puskesmas</span>
          </div>
        {/if}
      </div>
            ...
            <ul class="space-y-0.5 {section.title && !sidebarCollapsed ? 'mt-1' : (section.title ? 'mt-2' : 'mt-0')}">
              {#each section.items as item}
                <li>
                  ...
                  >
                    <span class="flex-shrink-0 {isActive(item.href) ? 'text-emerald-600' : 'text-gray-400 group-hover:text-gray-600'}">
                      {#if item.icon === 'home'}
                      ...
                      {:else if item.icon === 'tablet'}
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75"><path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5h3m-6.75 2.25h10.5a2.25 2.25 0 0 0 2.25-2.25v-15a2.25 2.25 0 0 0-2.25-2.25H6.75A2.25 2.25 0 0 0 4.5 4.5v15a2.25 2.25 0 0 0 2.25 2.25Z" /></svg>
                      ...
                      {:else if item.icon === 'hospital-bed'}
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.75"><path stroke-linecap="round" stroke-linejoin="round" d="M9 17.25v1.007a3 3 0 0 1-.879 2.122L7.5 21h9l-.621-.621A3 3 0 0 1 15 18.257V17.25m6-12V15a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 15V5.25m18 0A2.25 2.25 0 0 0 18.75 3H5.25A2.25 2.25 0 0 0 3 5.25m18 0V12a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 12V5.25M7.5 10.5h.008v.008H7.5V10.5Zm3 0h.008v.008H10.5V10.5Zm3 0h.008v.008H13.5V10.5Zm3 0h.008v.008H16.5V10.5Z" /></svg>
                    ...
                    {#if isActive(item.href) && !sidebarCollapsed}
                      <span class="ml-auto w-1.5 h-1.5 rounded-full bg-emerald-600 flex-shrink-0"></span>
                    {/if}
                  </a>
                </li>
              {/each}
      ...
      <div class="border-t border-gray-200 p-3 {sidebarCollapsed ? 'px-2' : ''}">
        {#if !sidebarCollapsed}
          <div class="flex items-center gap-3 px-2 py-2">
            <div class="flex items-center justify-center w-9 h-9 rounded-full bg-emerald-100 text-emerald-700 text-sm font-bold flex-shrink-0">
              {userInitials}
            </div>
    ...
    <div class="flex flex-col flex-1 {sidebarCollapsed ? 'lg:ml-[72px]' : 'lg:ml-64'} transition-all duration-300">
      <header class="sticky top-0 z-30 flex items-center h-16 px-4 bg-white/80 backdrop-blur-md border-b border-gray-200 gap-4">
        ...
        <div class="flex-1">
          <h1 class="text-lg font-semibold text-gray-900">SMARTHEALTH</h1>
        </div>
        ...
        <div class="flex items-center gap-2">
          ...
          <div class="relative">
            ...
            >
              <div class="flex items-center justify-center w-8 h-8 rounded-full bg-emerald-100 text-emerald-700 text-xs font-bold">
                {userInitials}
              </div>
